	...
	****************************************************************************/
	...
	Webtrends.dcs.prototype = {
		...
		*/
		...
		isReady: function () {
			return (this.plugInWaitCount <= 1="" 0);="" },="" *="" ***********************="" checks="" to="" see="" if="" all="" plugins="" have="" been="" loaded="" before="" calling="" setready="" checkready:="" function="" ()="" {="" (this.pluginwaitcount="" <="0)" this.setready();="" it="" flushs="" the="" commands(aka="" hits)="" queued="" up="" in="" individual="" tag's="" queue.="" flushing="" them="" just="" calls="" doaction="" method.="" then="" redirectqueue="" which="" replaces="" command="" array's="" push="" method="" with="" doaction,="" so="" no="" more="" commands="" get="" up.="" setready:="" (this._readyset)="" return;="" _fireevent('onready',="" this);="" this.flushqueue();="" this.redirectqueue();="" this._readyset="true;" takes="" each="" cmd="" objects="" and="" passes="" removes="" from="" array(queue).="" object="" has="" members="" {action:"functionnametocall",="" message:"anobjecttopassasanargument"}="" flushqueue:="" for="" (var="" i="0;" this.queue.length;="" i++)="" this.doaction(this.queue[i]);="" }="" this.queue="[];" queue(which="" is="" basically="" an="" array="" that="" "had"="" actions="" it)="" it's="" a="" new="" one,="" directly.="" expected="" you="" called="" flushqueue="" redirectqueue:="" var="" dcs="this;" this.queue.push="function" (cmd)="" dcs.doaction(cmd);="" };="" addtransform:="" (f,="" event)="" webtrends.addtransform(f,="" event,="" uses="" native="" queryselector,="" jquery,="" sizzle,="" yui="" selector="" engines="" selectively="" find="" attach="" events="" multitrack="" addselector:="" (selector,="" o_in)="" this="" basic="" empty="" object,="" should="" every="" key.="" o_base="{" 'domevent':="" 'click',="" 'callback':="" undefined,="" 'argsa':="" [],="" 'args':="" {},="" 'delaytime':="" 'transform':="" 'filter':="" 'actionelems':="" 'a':="" 1,="" 'input':="" can="" pass="" tagnames="" additional="" elements.="" tag="" names="" must="" be="" uppercase.="" 'finish':="" undefined="" o_args="Webtrends.extend(o_base," o_in,="" true);="" merge="" base="" _bindevent("wtmouse",="" (dcs_ignore,="" o_evt)="" ignore="" dcs_in.="" instead="" we="" will="" do="" closure="" around="" creator's="" dcs.addselectorreal(dcs,="" selector,="" webtrends.extend(o_evt,="" o_args,="" true));="" event="" input="" object.="" dcs);="" return="" this;="" sendselectortrack:="" (dcs,="" o,="" srcelm,="" type)="" o['element']="srcElm;" (type="==" "form"="" ||="" type="==" "input"="" 'button')="" o['domevent']="submit" ;="" dcs._multitrack(o);="" addselectorreal:="" o)="" (find="" &&="" o['event']="" o['actionelems'])="" srcelm="Webtrends.elemOfEvent(o["event"]," o['actionelems']);="" ?="" srcelm.tagname.tolowercase()="" :="" "";="" *in="" case="" of="" simple="" don't="" bother="" looping="" through="" all*="" (selector.touppercase()="" o['actionelems']="" selector.touppercase()="==" type.touppercase())="" this.sendselectortrack(dcs,="" type);="" els="find(selector)," els2="[];" (!els="" !srcelm)="" nothing="" found="" (els="" els.length)="" els.length;="" (els[i]="==" srcelm)="" break;="" name="" store="" returns="" key="" value="" pairs="" store.="" _dcsgetcookie:="" (name,="" object)="" element="extendArray(_document.cookie.split(";" ")).filter(function="" (el)="" el.indexof(name="" +="" '=") != -1;
			})[0];
			if (!element || element.length < name.length + 1) return false;
			var crumbs = element.split(name + " =')[1].split(':');="" extendarray(crumbs).foreach(function="" (a)="" b="a.split('=');" object[b[0]]="b[1];" map="" });="" true;="" added="" legacy="" support="" dcsgetcookie:="" function(name){="" cookies="_document.cookie.split(";" ");="" cmatch="[];" idx="0;" namelen="name.length;" clen="cookies.length;" (i="0;i<clen;i++){" c="cookies[i];" ((c.substring(0,namelen+1))="=(name+"=")){" cmatch[idx++]="c;" cmatchcount="cmatch.length;" (cmatchcount="">0){
				idx=0;
				if ((cmatchCount>1)&&(name==this.FPCConfig.name)){
					var dLatest=new Date(0);
					for (i=0;i<cmatchcount;i++){ var="" lv="parseInt(this.dcsGetCrumb(cmatch[i],"lv"));" dlst="new" date(lv);="" if="" (dlst="">dLatest){
		...
		dcsFPC: function () {
			...
			if (!this._dcsGetCookie(name, c)) {
				if (this.gTempWtId.length) {
					...
					for (var i = 2; i <= (32="" -="" curt.length);="" i++)="" {="" wt["co_f"]="" +="Math.floor(Math.random()" *="" 16.0).tostring(16);="" }="" wt["vt_f"]="1" ;="" if="" (this.gwtaccountrollup.length="=" 0)="" wt["vt_f_a"]="1" wt["vt_f_s"]="WT["vt_f_d"]" =="" "1";="" wt["vt_f_tlh"]="WT["vt_f_tlv"]" "0";="" else="" var="" id="c["id"];" lv="parseInt(c["lv"]);" ss="parseInt(c["ss"]);" ((id="=" null)="" ||="" (id="=" "null")="" isnan(lv)="" isnan(ss))="" return;="" dlst="new" date(lv);="" adj)="" 1000);="" dses.settime(ss);="" ((dcur.gettime()=""> (dLst.getTime() + 1800000)) || (dCur.getTime() > (dSes.getTime() + 28800000))) {
		...
		addDownloadTracking: function () {
			this.addSelector('a', {
				...
				transform: function (dcsObject, o) {
					...
					dcsObject._autoEvtSetup(o);
					var res = dcsObject.getURIArrFromEvent(el);
					var tmpDcsuri = res.dcsuri;
					if (tmpDcsuri.charAt(tmpDcsuri.length-1) == '/')
					{
						tmpDcsuri = tmpDcsuri.substring(0,tmpDcsuri.length-1);
					}
					var ttl = dcsObject.getTTL(e, el, res.dcsuri);
					o['argsa'].push(
						"DCS.dcssip", res.dcssip,
						"DCS.dcsuri", tmpDcsuri,
						"DCS.dcsqry", res.dcsqry,
						"DCS.dcsref", res.dcsref,
						"WT.ti", "Download:" + ttl,
						"WT.nv", dcsObject.dcsNavigation(e, dcsObject.navigationtag),
